...
const AddShows = () => {
    ...
    const handleDateTimeAdd = () => {
      ...
      const time = dateTime.toTimeString().substring(0, 5); // Format HH:MM
    ...
    return nowPlayingMovies.length > 0 ? (
      <>
        ...
        <div className='overflow-x-auto pb-4'>
          <div className='group grid grid-cols-7 gap-4 mt-4 w-max min-w-full'>
            {nowPlayingMovies.map((movie) => (
              <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300`} onClick={()=> setSelectedMovie(movie.id)}>
                <div className='relative rounded-lg overflow-hidden'>
                  <img src={movie.poster_path} alt="" className='w-full object-cover brightness-90 aspect-[2/3]'/>
